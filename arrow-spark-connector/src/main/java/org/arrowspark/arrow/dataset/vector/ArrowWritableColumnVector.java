 ...
 */
/*
 * Note: The following was modified from: https://github.com/Intel-bigdata/OAP
 * Modifications: Added IEEE 754 floating-point little-endian support.
 * All questions/issues regarding this file should go to https://github.com/Intel-bigdata/OAP.
 * All credit, save for the modifications, goes to (the original creators at) Intel.
 */

package org.arrowspark.arrow.dataset.vector;
...
public final class ArrowWritableColumnVector extends WritableColumnVector {
    ...
    private boolean allowOffHeap;

    @SuppressWarnings("DeprecatedIsStillUsed")
    @Deprecated
    public ArrowWritableColumnVector(ValueVector vector, int ordinal, int capacity, boolean init, boolean allowOffHeap) {
        this(vector, null, ordinal, capacity, init, allowOffHeap);
    }

    public ArrowWritableColumnVector(ValueVector vector, ValueVector dicionaryVector, int ordinal, int capacity, boolean init, boolean allowOffHeap) {
        super(capacity, ArrowUtils.fromArrowField(vector.getField()));
        this.allowOffHeap = allowOffHeap;
    ...
    public ArrowWritableColumnVector(int capacity, DataType dataType, boolean allowOffHeap) {
        super(capacity, dataType);
        this.allowOffHeap = allowOffHeap;
        vectorCount.getAndIncrement();
        refCnt.getAndIncrement();
        String timeZoneId = SQLConf.get().sessionLocalTimeZone();
        List<Field> fields = Collections.singletonList(ArrowUtils.toArrowField("col", dataType, true, timeZoneId));
        Schema arrowSchema = new Schema(fields);
        VectorSchemaRoot root = VectorSchemaRoot.create(arrowSchema, MemoryUtil.getAllocator(allowOffHeap));
    ...
    public static BufferAllocator getAllocator() {
        return MemoryUtil.getAllocator();
    }
    public static BufferAllocator getAllocator(boolean allowOffHeap) {
        return MemoryUtil.getAllocator(allowOffHeap);
    }
     ...
     */
    public static ArrowWritableColumnVector[] allocateColumns(int capacity, StructType schema, boolean allowOffHeap) {
        String timeZoneId = SQLConf.get().sessionLocalTimeZone();
        Schema arrowSchema = ArrowUtils.toArrowSchema(schema, timeZoneId);
        VectorSchemaRoot new_root = VectorSchemaRoot.create(arrowSchema, MemoryUtil.getAllocator(allowOffHeap));
        ...
        for (int i = 0; i < fieldVectors.size(); i++) {
            vectors[i] = new ArrowWritableColumnVector(fieldVectors.get(i), i, capacity, true, allowOffHeap);
        }
        // LOG.info("allocateColumns allocator is " + allocator);
        return vectors;
    }

    public static ArrowWritableColumnVector[] loadColumns(int capacity, List<FieldVector> fieldVectors, List<FieldVector> dictionaryVectors, boolean allowOffHeap) {
        ...
        ArrowWritableColumnVector[] vectors = new ArrowWritableColumnVector[fieldVectors.size()];
        for (int i = 0; i < fieldVectors.size(); i++)
            vectors[i] = new ArrowWritableColumnVector(fieldVectors.get(i), dictionaryVectors.get(i), i, capacity, false, allowOffHeap);
        return vectors;
    }

    public static ArrowWritableColumnVector[] loadColumns(int capacity, List<FieldVector> fieldVectors, boolean allowOffHeap) {
        ArrowWritableColumnVector[] vectors = new ArrowWritableColumnVector[fieldVectors.size()];
        for (int i = 0; i < fieldVectors.size(); i++) {
            vectors[i] = new ArrowWritableColumnVector(fieldVectors.get(i), i, capacity, false, allowOffHeap);
        }
        return vectors;
    }

    public static ArrowWritableColumnVector[] loadColumns(int capacity, Schema arrowSchema, ArrowRecordBatch recordBatch, boolean allowOffHeap) {
        return loadColumns(capacity, arrowSchema, recordBatch, null, allowOffHeap);
    }

    public static ArrowWritableColumnVector[] loadColumns(int capacity, Schema arrowSchema, ArrowRecordBatch recordBatch, BufferAllocator _allocator, boolean allowOffHeap) {
        if (_allocator == null)
            _allocator = MemoryUtil.getAllocator(allowOffHeap);
        VectorSchemaRoot root = VectorSchemaRoot.create(arrowSchema, _allocator);
        VectorLoader loader = new VectorLoader(root);
        loader.load(recordBatch);
        return loadColumns(capacity, root.getFieldVectors(), allowOffHeap);
    }
    ...
    private void createVectorAccessor(ValueVector vector, ValueVector dictionary) {
        if (dictionary != null) {
            if (!(vector instanceof IntVector)) {
                throw new IllegalArgumentException("Expect int32 index vector. Found: " + vector.getMinorType());
            }
            ...
            if (dictionary instanceof VarBinaryVector) {
                ...
                throw new IllegalArgumentException("Unrecognized index value type: " + dictionary.getMinorType());
            }
            return;
        }
    ...
    protected ArrowWritableColumnVector reserveNewColumn(int capacity, DataType type) {
        return new ArrowWritableColumnVector(capacity, type, allowOffHeap);
    }
    ...
    public void putFloatsLittleEndian(int rowId, int count, byte[] src, int srcIndex) {
        writer.setFloatsLittleEndian(rowId, count, src, srcIndex);
    }
    ...
    private static class StringAccessor extends ArrowVectorAccessor {
        ...
        final UTF8String getUTF8String(int rowId) {
            ...
            if (stringResult.isSet == 0) {
                ...
                return UTF8String.fromAddress(null, stringResult.buffer.memoryAddress() + stringResult.start, stringResult.end - stringResult.start);
            }
        }
    }
    ...
    private abstract static class ArrowVectorWriter {
        ...
        void setFloatsLittleEndian(int rowId, int count, byte[] src, int srcIndex) {
            throw new UnsupportedOperationException();
        }
    ...
    private static class FloatWriter extends ArrowVectorWriter {
        ...
        void setFloatsLittleEndian(int rowId, int count, byte[] src, int srcIndex) {
            int srcOffset = srcIndex + Platform.BYTE_ARRAY_OFFSET;
            for (int i = 0; i < count; i++, srcOffset += 4) {
                float tmp = Platform.getFloat(src, srcOffset);
                if (bigEndianPlatform) {
                    int converted = Float.floatToIntBits(tmp);
                    tmp = Float.intBitsToFloat(java.lang.Integer.reverseBytes(converted));
                }
                writer.setSafe(rowId + i, tmp);
            }
        }
    ...
    private static class DecimalWriter extends ArrowVectorWriter {
        ...
        final void setInt(int rowId, int value) {
            ...
            writer.setSafe(rowId, v.setScale(writer.getScale(), RoundingMode.UNNECESSARY));
        }
